...
// $("#myDiv") >> jQuery("#myDiv")로 호출되고, a에는 #myDiv, c는 undefined값이 설정.
// jQuery함수를 정의한다.
function jQuery(a, c) {

  // Shortcut for document ready (because $(document).each() is silly)
  // constructor >> a의 생성자를 확인하는 여기서는 String이된다. 
  // 밑은 a인자가 함수 일 경우를 뜻한다. 
  if (a && a.constructor == Function && jQuery.fn.ready)
    return jQuery(document).ready(a);

  // Make sure that a selection was provided
  // 인자 a의 디폴트값을 저장하는 코드이다.
  // a가 있으므로 뒷부분은 생략.
  a = a || jQuery.context || document;

  // Watch for when a jQuery object is passed as the selector
  // a는 문자열, a.jQuery같이 jquery 프로퍼티를 가지는 객체가 아니다. 
  if (a.jquery)
    return $(jQuery.merge(a, []));

  // Watch for when a jQuery object is passed at the context
  // c가 jquery 프로퍼티를 가진 객체인지를 확인한다. 
  if (c && c.jquery)
    return $(c).find(a);

  // If the context is global, return a new object
  // this가 어떻게 바인딩 되었는지 확인, this가 전역 객체 window로 바인딩되는 경우 > jquery()를 함수형태로 호출하는 경우이다.
  // $("myDiv")는 함수호출형태이므로 this는 전역객체인 window에 바인딩된다.
  if (window == this)
    return new jQuery(a, c);

  // Handle HTML strings
  // 아래는 정규표현식이라고 한다.
  // '/'는 정규표현식 리터럴을 만드는 기호이다. 
  // ^ - 문자열의 시작을 나타낸다.
  // [^<]* - 문자클래스([])안에 포함된 ^는 지정한 문자를 제외하라는 의미이다. 즉,'<'문자를 제외한 모든 문자를 의미.
  // 클래스뒤에 *(수량자) - 문자 클래스에 해당하는 글자가 0번 이상나타나는 것을 의미, 
  // 마지막으로 빈 문자열이나 '<'문자를 제외한 문자나 문자열을 의미한다.
  // () 캡쳐 그룹을 나타낸다. >> <.+>는 <>로 둘려싸인 문자나 문자열을 의미한다 즉, HTML 태그 형태를 캡쳐하는 정규표현식이다.
  // [^>]* 빈문자열이나, '>'문자를 제외한 문자나 문자열.
  // $ 문자열의 끝.
  var m = /^[^<]*(<.+>)[^>]*$/.exec(a);
  // 빈 문자열이나, '<'문자를 제외한 문자나 문자열로 시작하고,
  // 중간에 태그(<>) 형태의 문자나 문자열이 있으며,
  // 빈 문자열이나, '>'문자를 제외한 문자나 문자열로 끝난다.

  // exec메소드
  // regexp.exec(String) 메서드
  // 정규표현식 객체의 exec(String) 메서드는 인자로 받은 String이 자신의 정규표현식에 일치하는지를 체크하고, 일치할 경우 배열을 반환한다. 일치하지않으면 null반환
  if (m) a = jQuery.clean([m[1]]);

  // Watch for when an array is passed in
  // get >> 메시드를 호출하는 코드.
  // ? true >> jQuery.merge / false >> jQuery.find 메서드가 호출된다.
  // a는 String >> nodeType이 없다.
...
// 일반적으로 jQuery를 사용할 때, $()형태로 jQuery의 기능을 맵핑한다.
...
// jQuery extend()메서드 - 객체 확장
jQuery.extend = jQuery.fn.extend = function (obj, prop) {
  if (!prop) {
    prop = obj;
    obj = this;
  }
  // obj인자만 넘길경우 위 조건문이 참으로 판명, obj로 전달된 객체를 prop에 저장, 그리고 obj에 this를 저장.
  // 함수호출패턴에 따라, 다르게 바인딩된다.
  // jQuery 함수 객체에서 extend() 호출, this >> jQuery함수객체로 바인딩.
  // jQuery.prototype객체에서 extend()호출 this >> jQuery.prototype 객체로 바인딩.  
  for (var i in prop) obj[i] = prop[i]; // prop 인자의 모든 프로퍼티를 obj인자로 복사. >> obj객체에 prop객체의 프로퍼티가 추가된다.(본기능)
  return obj;
};
...
// jQuery.extend() >> jQuery 함수 객체
// jQuery.fn.extend() >> jQuery.prototype 객체의 기능을 확장
jQuery.fn.extend({
